...
type
  TQrCode = class
    class procedure Gerar(data: string; stm: TStream);
  end;
...
class procedure TQrCode.Gerar(data: string; stm: TStream);
var
  lEncodeURL: string;
  lHInet: Pointer;
  lBuffer: Pointer;
...
begin
  lEncodeURL := Format(QR_URL, [200, 200, 'M', TNetEncoding.URL.Encode(data)]);
      ...
      begin
        repeat
          ...
          if lBytesReaded > 0 then
            Stm.Write(lBuffer^, lBytesReaded);
